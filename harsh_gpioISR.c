...
// GPIO switch/button masks
#define GPIO_LEFT_MASK   0x01
#define GPIO_RIGHT_MASK  0x02
#define GPIO_DROP_MASK   0x04
...
static unsigned int prev_gpio = 0;
...
void game_init(void)
{
    ...
    printf("\n---- Connect Four Game ----");
    printf("\nGPIO left  : move left");
    printf("\nGPIO right : move right");
    printf("\nGPIO drop  : drop piece");
    printf("\nGPIO reset : reset game");
    printf("\nGPIO quit  : quit game");
    printf("\nP1 = RED, P2 = GREEN\n");
...
void handle_drop(void)
{
    ...
    if (check_winner(row, cursor_col)) {
        game_over = 1;
...
//--------------------------------------------------------
// UART ISR kept for optional keyboard control
//--------------------------------------------------------
void UART_ISR(void)
{
    char ch;
